...
std::string	response::form_error_page(std::string code) {
	...
	this->header += std::to_string(body.length());
	this->header += "\nConnection: close\n\n";
	this->header += body;
...
void	response::form_header(std::string content_type, std::string uri, int client) {
	...
	fs.seekg(0, fs.beg);
	fs.close();
	this->header += std::to_string(length);
	this->header += "\nConnection: close\n\n";
...
std::string response::parse_multipart(std::string pwd, int client) {
	...
	std::ofstream fs;
	fs.open(name, std::ofstream::out | std::ofstream::binary);
	fs.write(subbody.c_str(), subbody.length());
	fs.close();
	this->header += "\nConnection: close\n\n";
	// this->from_str_to_char(this->header.length() + subbody.length());
...
std::string response::parse_binary(std::string full_path, int client, std::string type) { 
	...
	std::string file_name = "/cgi-bin/file/downloads/file" + std::to_string(id);
	...
	close(fd);
	std::ofstream fs;
	fs.open(file_name, std::ofstream::out | std::ofstream::binary);
...
void	response::do_partial_content(char *range, std::string &location) {
	...
	fs.open(location, std::ios::binary);
	if (!fs.is_open()) {
		form_error("404");
		return ;
	}
	...
	int length = fs.tellg();
	fs.seekg(0, fs.beg);
	this->header += std::to_string(length - 1);
	this->header += "/";
	this->header += std::to_string(length);
	this->header += "\nContent-Length: ";
	unsigned int len;
...
response::response(const response &to_copy) {
	...
	for (i = 0; to_copy.envp[i]; i++) {
		int j = std::strlen(to_copy.envp[i]) + 1;
