...
char** add_env(std::string s1)
{
	...
	array_env[map_env.size()] = nullptr;
	i = 0;
	for (std::map<std::string, std::string>::const_iterator it_array = map_env.cbegin(); it_array != map_env.cend(); it_array++, i++)
...
bool get_body(std::string s1, std::string *body) {
    ...
    if((pos = s1.find("Transfer-Encoding: chunked", 0)) != -1){
        return(get_body_chunked(s1, body));
    }
    pos = 0;
    pos = s1.find("\r\n\r\n", pos);
    pos += 4;
...
bool get_body_chunked(std::string s1, std::string *body){
    int pos = 0;
    int res = 0;
    unsigned int x = 0;
    if((pos = s1.find("\r\n\r\n", pos)) != -1)
